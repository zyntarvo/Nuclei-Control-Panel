#!/usr/bin/env python3
"""
Nuclei Control Panel installer (tkinter GUI).

  INSTALL          — fresh Ubuntu: Go (apt golang-go, not the broken go.dev tarball),
                     projectdiscovery/nuclei + nuclei-templates, panel, systemd.
  INSTALL ONLY CP  — panel only. nuclei must already be on PATH.

This installer does NOT touch reconFTW. Nuclei is the product.

Usage:  python nuclei_setup.py
"""
...
PANEL_BUILD = "1.0.0"
REMOTE_PANEL = "/root/nuclei-panel"
REMOTE_TEMPLATES = "/root/nuclei-templates"
NUCLEI_GO = "github.com/projectdiscovery/nuclei/v3/cmd/nuclei@latest"
...
class Installer:
    ...
    def ensure_swap(self):
        ...
        if mem_kb and mem_kb < 3_500_000 and swap_rows <= 1:
            self.log("[*] RAM < 3.5G — adding 2G swap for Go compile")
            self.ssh.run(
                "if [ ! -f /swapfile ]; then "
    ...
    def apt_base(self):
        ...
        self.ssh.run(
            ...
            "build-essential unzip jq gcc make sudo",
            timeout=600,
            check=True,
        )

    def install_browser_deps(self):
        # Shared libraries required by the headless Chromium that nuclei's
        # -headless flag downloads via go-rod. Package names carry a t64
        # suffix on Ubuntu 24.04+/26.04 and none on 22.04, so try both.
        self.set_pct(14)
        self.log("[*] Headless browser runtime libs (for -headless templates)")
        pairs = [
        ...
        plain = (
            ...
            "libgbm1 libpango-1.0-0 libcairo2 libnss3 libnspr4 libdrm2 "
            "libxext6 libxi6 libxtst6 libx11-xcb1 libxcb1 libxshmfence1 "
            "libvulkan1 fonts-liberation ca-certificates"
        )
        tries = " ".join(
            f"apt-get install -y {new} || apt-get install -y {old} || true; "
            for new, old in pairs
        )
        self.ssh.run(
            "export DEBIAN_FRONTEND=noninteractive; "
            f"apt-get install -y {plain} || true; "
            + tries,
            timeout=900,
        )
    ...
    def path_env(self):
        return (
            ...
            "DEBIAN_FRONTEND=noninteractive HOME=/root GOPROXY=https://proxy.golang.org,direct"
        )
    ...
    def run_logged(self, title, cmd, logfile, timeout=3600):
        """Long job in its own systemd unit so SSH PTY close cannot SIGHUP it."""
        self.log(f"[*] {title}")
        unit = "ncp-" + os.path.basename(logfile).replace(".", "-")[:40]
        wrapper = f"/tmp/_panel_{unit}.sh"
        ...
        self.ssh.run(
            f"chmod +x {wrapper}; rm -f {exit_file}; : > {outer}; mkdir -p $(dirname {shlex.quote(logfile)})",
            pty=False, quiet=True,
        )
        start = (
    ...
    def install_nuclei(self):
        self.set_pct(28)
        env = self.path_env()
        log = "/tmp/nuclei-go-install.log"
        self.log("[*] go install nuclei (official ProjectDiscovery binary)")
        rc = self.run_logged(
            "go install nuclei",
            f"{env}; go install -v {NUCLEI_GO} >> {log} 2>&1",
            log,
            timeout=2400,
        )
        if rc != 0:
            raise RuntimeError(
                "go install nuclei failed. See /tmp/nuclei-go-install.log"
            )
        self.symlink_bins()
        code, out = self.ssh.run(f"{self.path_env()}; nuclei -version", timeout=40)
        if code != 0:
            raise RuntimeError("nuclei binary not runnable after install\n" + (out or "")[-800:])
        self.log("[+] " + (out or "").replace("\r", "").strip().splitlines()[-1][:160])

    def symlink_bins(self):
        self.set_pct(55)
        self.log("[*] Symlink /root/go/bin and ~/.local/bin -> /usr/local/bin")
        # Empty dirs make `for f in dir/*` fail (glob stays literal, last test is exit 1).
        self.ssh.run(
            "shopt -s nullglob; "
            ...
            "  done; "
            "done; "
            "if [ -x /root/go/bin/nuclei ]; then ln -sfn /root/go/bin/nuclei /usr/local/bin/nuclei; fi; "
            "command -v nuclei >/dev/null || test -x /usr/local/bin/nuclei",
            timeout=30,
            check=True,
        )
    ...
    def install_templates(self):
        self.set_pct(60)
        code, _ = self.ssh.run(f"test -d {REMOTE_TEMPLATES}/http", quiet=True)
        if code != 0:
            self.log(f"[*] Cloning {TEMPLATES_REPO}")
            self.ssh.run(
                f"rm -rf {REMOTE_TEMPLATES}.partial; "
                f"git clone --depth 1 {TEMPLATES_REPO} {REMOTE_TEMPLATES}.partial && "
                f"mv {REMOTE_TEMPLATES}.partial {REMOTE_TEMPLATES}",
                timeout=600,
                check=True,
            )
        else:
            self.log("[+] nuclei-templates already present")
        env = self.path_env()
        log = "/tmp/nuclei-templates-update.log"
        self.log("[*] nuclei -update-templates")
        rc = self.run_logged(
            "nuclei template update",
            f"{env}; nuclei -update-templates -ud {REMOTE_TEMPLATES} -duc >> {log} 2>&1 || true",
            log,
            timeout=1800,
        )
        if rc != 0:
            self.log("[!] template update returned non-zero — clone is enough, continuing")
        # nuclei looks for ~/.config/nuclei/.nuclei-ignore; a bare git clone
        # does not place it there, which prints an [ERR] on every scan. The
        # ignore list ships in the templates repo root, so copy it over.
        self.ssh.run(
            "mkdir -p /root/.config/nuclei; "
            f"[ -f {REMOTE_TEMPLATES}/.nuclei-ignore ] && "
            f"cp -f {REMOTE_TEMPLATES}/.nuclei-ignore /root/.config/nuclei/.nuclei-ignore || true",
            timeout=30,
        )
        self.ssh.run(f"touch {REMOTE_TEMPLATES}/.panel_install_done", quiet=True)
        self.log("[+] Templates at " + REMOTE_TEMPLATES)
    ...
    def upload_panel(self):
        ...
        need = ["app.py", "findings.py", "requirements.txt"]
        for n in need:
    ...
    def write_env(self):
        ...
        body = (
            ...
            f"NUCLEI_BIN=/usr/local/bin/nuclei\n"
            f"NUCLEI_TEMPLATES={REMOTE_TEMPLATES}\n"
            f"NUCLEI_SCANS={REMOTE_PANEL}/data/scans\n"
    ...
    def systemd(self):
        ...
        unit = f"""[Unit]
Description=Nuclei Control Panel
...
"""
        self.log("[*] systemd nuclei-panel")
        self.ssh.run(
            "cat > /etc/systemd/system/nuclei-panel.service <<'EOF'\n"
            + unit
            + "EOF\n"
            "mkdir -p /etc/systemd/system/nuclei-panel.service.d\n"
            "cat > /etc/systemd/system/nuclei-panel.service.d/kill.conf <<'K'\n"
            "[Service]\nKillMode=process\nK\n"
            "systemctl daemon-reload\n"
            "systemctl enable nuclei-panel\n"
            "systemctl restart nuclei-panel",
            timeout=40,
            check=True,
        )
        self.ssh.run(
        ...
        time.sleep(2)
        code, out = self.ssh.run("systemctl is-active nuclei-panel")
        if "active" not in out:
            _, j = self.ssh.run("journalctl -u nuclei-panel -n 30 --no-pager")
            raise RuntimeError("panel unit is not active\n" + j[-2000:])
        self.log("[+] nuclei-panel is active")
    ...
    def run_panel_only(self):
        self.detect()
        code, _ = self.ssh.run("command -v nuclei", quiet=True)
        if code != 0:
            raise RuntimeError(
                "nuclei not found on PATH. Use INSTALL (full stack), "
                "or install ProjectDiscovery nuclei first."
            )
        self.detect_go_if_present()
        self.ssh.run(
            "export DEBIAN_FRONTEND=noninteractive; "
            "apt-get update -y && apt-get install -y python3 python3-venv python3-pip python3-full git",
            timeout=300,
            check=True,
        )
        if self.goroot:
            self.symlink_bins()
        # headless (-headless templates) needs Chromium runtime libs even in
        # panel-only mode, otherwise the browser fails to launch
        self.install_browser_deps()
        code, _ = self.ssh.run(f"test -d {REMOTE_TEMPLATES}", quiet=True)
        if code != 0:
            self.log("[*] Templates missing — cloning nuclei-templates")
            self.install_templates()
        # ensure ~/.config/nuclei/.nuclei-ignore exists even if templates
        # were already present (avoids the [ERR] on every scan)
        self.ssh.run(
            "mkdir -p /root/.config/nuclei; "
            f"[ -f {REMOTE_TEMPLATES}/.nuclei-ignore ] && "
            f"cp -f {REMOTE_TEMPLATES}/.nuclei-ignore /root/.config/nuclei/.nuclei-ignore || true",
    ...
    def _done(self):
        ...
        self.log("  Engine:    nuclei (ProjectDiscovery)")
        self.log(f"  Templates: {REMOTE_TEMPLATES}")
        self.log("  Optional PDCP API key can be pasted in the panel.")
        self.log("=" * 54)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(f"NUCLEI  //  INSTALLER v{PANEL_BUILD}")
    ...
    def _build(self):
        ...
        if mark:
            try:
                ...
                pass
        tk.Label(head, text="NUCLEI", fg=ACCENT, bg=BG, font=TITLE).pack(pady=(8, 0))
        tk.Label(
            head,
        ...
        try:
            ...
            pass
        style.configure("NC.Horizontal.TProgressbar", troughcolor=BG3, background=ACCENT, bordercolor=BG)
        self.pb = ttk.Progressbar(self, style="NC.Horizontal.TProgressbar", mode="determinate", maximum=100)
        ...
        self._log("[*] INSTALL = Go + nuclei + nuclei-templates + panel")
        self._log("[*] INSTALL ONLY CP = panel only, nuclei must already be on PATH")
        self._log("[*] This is NOT reconFTW. Engine = projectdiscovery/nuclei.")
